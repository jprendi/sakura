...
if __name__ == "__main__":
    try:
        print(f"--- Starting EE Relative Diff Maps for {output_pdf} ---")

        with PdfPages(output_pdf) as pdf:
            for hist_name in target_histograms:
                print(f" > Processing {hist_name}...")

                if "Plus"    in hist_name: ee_side = "EE+"
                elif "Minus" in hist_name: ee_side = "EE-"
                else:                      ee_side = "EE"

                xlabel = rf"RecHit {ee_side} $ix$"
                ylabel = rf"RecHit {ee_side} $iy$"
                ...
                # Precompute radial grid and bins (same for all pairs)
                ix_centers = 0.5 * (xedges[:-1] + xedges[1:])
                iy_centers = 0.5 * (yedges[:-1] + yedges[1:])
                ix_grid, iy_grid = np.meshgrid(ix_centers, iy_centers, indexing='ij')
                r_grid      = np.sqrt((ix_grid - BEAM_CENTER[0])**2 + (iy_grid - BEAM_CENTER[1])**2)
                ...
                for label_a, label_b in diff_pairs:
                    ...
                    # --- Figure: 2 rows (same layout as original) ---
                    ...
                    cbar.set_label(f"Rel. Diff. ({label_a} - {label_b}) / {label_b}", fontsize=18)
                    cbar.formatter = plt.FuncFormatter(lambda val, pos: f"{val:.2f}") 
                    cbar.update_ticks()

                    # --- Radial projection (True Rel Diff of the sums) ---
                    # 1. Sum raw hits in radial bins for A
                    sum_a_r, r_edges = np.histogram(r_vals, bins=r_bins, weights=data_a.flatten())
                    
                    # 2. Sum raw hits in radial bins for B
                    sum_b_r, _ = np.histogram(r_vals, bins=r_bins, weights=data_b.flatten())

                    # 3. Compute relative difference of the sums
                    with np.errstate(divide='ignore', invalid='ignore'):
                        rel_diff_r = np.where(sum_b_r != 0, (sum_a_r - sum_b_r) / sum_b_r, np.nan)

                    hep.histplot(rel_diff_r, bins=r_edges, ax=ax_r,
                    ...
                    #ax_r.set_xlim(r_edges[0], r_edges[-1])
                    ax_r.set_xlim(0,60)
                    divider_r = make_axes_locatable(ax_r)
                    divider_r.append_axes("right", size="5%", pad=0.05).axis("off")

                    # --- Save ---
                    pdf.savefig(fig, bbox_inches='tight')
                    ax.set_title("")
                    png_name = f"RelDiff_{label_a}_vs_{label_b}_{hist_name}_rad2D.png"
